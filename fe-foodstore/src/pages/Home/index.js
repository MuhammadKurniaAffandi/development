...
export default function Home() {
  // baca state `products` dari Redux store
  let products = useSelector((state) => state.products);
  // baca state cart dari redux store
        ...
        content={
          <div className="md:flex md:flex-row-reverse w-full mr-5 h-full min-h-screen">
            <div className="w-full md:w-3/4 pl-5 pb-10">
              ...
              {/* Komponent untuk input pencarian / keyword */}
              ...
              {/* komponen untuk pencarian berdasarkan tags */}
                ...
                {products.data.map((product, index) => {
                  return (
                    <div key={index} className="p-2">
                      <CardProduct
                        title={product.name}
                        imgUrl={`${process.env.REACT_APP_API_HOST}/upload/${product.image_url}`}
                        price={product.price}
